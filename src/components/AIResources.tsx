  ...
  const tools: ToolCardProps[] = [
    {
      ...
      description: 'Genera variantes de preguntas alineadas a niveles de complejidad cognitiva.',
      icon: <BrainCircuit size={24} />,
      color: 'bg-emerald-500',
      inputs: [
        { name: 'topic', label: 'Tema de la Pregunta', placeholder: 'Ej: Algoritmos de ordenamiento' },
        { name: 'course', label: 'Asignatura', placeholder: 'Ej: Programación I' }
      ]
    },
    {
      toolId: 'evaluator',
      title: 'Evaluador de Exámenes',
      description: 'Analiza la validez pedagógica y técnica de tus instrumentos.',
      icon: <ClipboardCheck size={24} />,
      color: 'bg-blue-600',
      inputs: [
        { name: 'content', label: 'Contenido del Examen/Ítem', placeholder: 'Pega aquí tus preguntas...', type: 'textarea' }
      ]
    },
    {
      toolId: 'rubric',
      title: 'Generador de Rúbricas',
      description: 'Crea matrices dinámicas para evaluar desempeños y competencias.',
      icon: <History size={24} />,
      color: 'bg-amber-500',
      inputs: [
        { name: 'topic', label: 'Tema / Actividad', placeholder: 'Ej: Proyecto final de base de datos' },
        { name: 'activity', label: 'Tipo de Actividad', placeholder: 'Ej: Exposición, Informe, Código' }
      ]
    },
    {
      toolId: 'improver',
      title: 'Mejorador de Preguntas',
      description: 'Optimiza enunciados y distractores para mayor claridad técnica.',
      icon: <FileEdit size={24} />,
      color: 'bg-slate-900',
      inputs: [
        { name: 'question', label: 'Ítem a Mejorar', placeholder: 'Pega aquí la pregunta...', type: 'textarea' }
      ]
    }
  ];
  ...
  return (
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom duration-700 py-10">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-slate-100 pb-10">
        <div>
          <h1 className="text-4xl font-black text-slate-900 uppercase tracking-tighter leading-tight">Caja de Herramientas <span className="text-brand-primary">IA UniCordoba</span></h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {tools.map((tool) => (
            <motion.div 
              ...
              className="card p-8 flex flex-col gap-6 group cursor-pointer border-b-4 border-b-slate-100 hover:border-b-brand-primary transition-all duration-300"
              onClick={() => {
        ...
        <div className="animate-in fade-in zoom-in-95 duration-500">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            <div className="lg:col-span-1 space-y-6">
              <div className="card p-8 space-y-6 bg-white shadow-xl">
                <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
                  <div className={`w-10 h-10 ${tools.find(t => t.toolId === activeTool)?.color} rounded-xl flex items-center justify-center text-white shadow-md`}>
                    {tools.find(t => t.toolId === activeTool)?.icon}
                  </div>
                  <h2 className="text-lg font-black text-slate-900 uppercase tracking-tight">{tools.find(t => t.toolId === activeTool)?.title}</h2>
                </div>
                
                <div className="space-y-4">
                  {tools.find(t => t.toolId === activeTool)?.inputs.map(input => (
                ...
                <button 
                  ...
                  disabled={loading || Object.values(formValues).some(v => !v)}
            ...
            <div className="lg:col-span-2">
              <div className="card p-8 min-h-[400px] border-dashed border-2 flex flex-col relative overflow-hidden">
                <div className="absolute top-0 right-0 p-4">
                   <div className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Área de Resultados</div>
                </div>
